...
public class GlobalPlatform {
    ...
    //public static EnumSet<APDUMode> defaultMode = EnumSet.of(APDUMode.CLR);
     ...
     */
    public GlobalPlatform(CardChannel channel) throws Exception {
        if (channel == null) {
    ...
    public static void handlerWrap(SCPWrapper wraper, CommandAPDU command) throws GPException {
        CommandAPDU wc = wraper.wrap(command);
    }

    public static void testWrap(SCPWrapper wraper, CommandAPDU command) throws GPException {
        CommandAPDU wc = wraper.wrap(command);
    }

    public static byte[] getSendBytes(SCPWrapper wraper, CommandAPDU command) throws GPException {
        CommandAPDU wc = wraper.wrap(command);
        return wc.getBytes();
    }

    public static byte[] getWrapperIV(SCPWrapper wraper, CommandAPDU command) throws GPException {
        CommandAPDU wc = wraper.wrap(command);
        GlobalPlatform.SCP0102Wrapper wrapperGP = (GlobalPlatform.SCP0102Wrapper) wraper;
        return wrapperGP.getIV();
    }
...
//			giveStrictWarning("Package with AID " + cap.getPackageAID() + " is already present on card");
            // TODO: 2017/9/12 小阿威修改  就算没有删除成功，也不能挂掉，写包前提：保证cap包加载成功
            L.e("Package with AID " + cap.getPackageAID() + " is already present on card");
        }
    ...
    public static class SCP0102Wrapper extends SCPWrapper {
        ...
        public SCP0102Wrapper(GPKeySet sessionKeys, int scp, EnumSet<APDUMode> securityLevel, byte[] icv, byte[] ricv, int bs) {
            this.blockSize = bs;
    ...
    public static class SCP03Wrapper extends SCPWrapper {
        ...
        public SCP03Wrapper(GPKeySet sessionKeys, int scp, EnumSet<APDUMode> securityLevel, byte[] icv, byte[] ricv, int bs) {
            this.sessionKeys = sessionKeys;
            this.blockSize = bs;
