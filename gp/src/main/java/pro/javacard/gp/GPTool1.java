...
public final class GPTool1 {
    ...
    public static void main(String[] argv, CardChannel channel) throws Exception {
        /*argv = new String[]{"-package","A000000333FE00000000000666600000","-applet",
                "A000000333FE00000000000666600001","-create","A0000000035350410000000000000001","-param",
				"C90D810202158201F88301F88701F8EF0AA008810101A5038201C0","-d" };*/
        ...
        try {
            ...
            if (needsAuthentication(args)) {
                ...
                if (args.has(OPT_SC_MODE)) {
                    mode.clear();
                    mode.add((APDUMode) args.valueOf(OPT_SC_MODE));
                }
                ...
                if (args.has(OPT_LOAD)) {
                    ...
                    try {
                        //todo 小阿威修改 加载cap包前加入新旧卡的判断
                        AID aid = loadcap.getPackageAID();
                        if (!gp.getRegistry().allAIDs().contains(aid)) {
                            System.out.println("这是一张新卡,package AID为:" + aid);
                        } else {
                            System.out.println("这是一张旧卡,package AID为:" + aid);
                            gp.deleteAID(aid, true);
                            System.out.println(aid + " deleted.");
                        }
                ...
                if (args.has(OPT_UNLOCK)) {
                    // Write default keys
                    List<GPKey> newkeys = new ArrayList<GPKey>();
                ...
                if (args.has(OPT_LOCK)) {
                    ...
                    // Add into a list
                    List<GPKey> updatekeys = new ArrayList<GPKey>();
                    updatekeys.add(new GPKey(new_version, 01, newkeys.getKey(KeyType.ENC)));
                    updatekeys.add(new GPKey(new_version, 02, newkeys.getKey(KeyType.MAC)));
                    updatekeys.add(new GPKey(new_version, 03, newkeys.getKey(KeyType.KEK)));
