...
public class Iso7816 {
	...
	public static class Response extends Iso7816 {
		...
		public String getSWStr() {
			return Integer.toHexString(getIntSW1() << 8 | getIntSW2());
		}

		public int getSW() {
			return getIntSW1() << 8 | getIntSW2();
		}

		public int getIntSW1() {  return data[data.length - 2] & 255;}

		public int getIntSW2() {
	...
	public final static class StdTag {
		...
		public Response testSelectByName() throws IOException {
			ByteBuffer buff = ByteBuffer.allocate(mName.length + 6);
			buff.put((byte) 0x00) // CLA Class
					.put((byte) 0xA4) // INS Instruction
					.put((byte) 0x04) // P1 Parameter 1
					.put((byte) 0x00) // P2 Parameter 2
					.put((byte) mName.length) // Lc
					.put(mName).put((byte) 0x00); // Le

			return new Response(transceive(buff.array()));
		}


		public Response selectSD() throws IOException {
			// 00A40400 0E 325041592E5359532E4444463031
		...
		public Response selectAID(byte[] aid) throws IOException {
			// 00A40400 0E 325041592E5359532E4444463031
			ByteBuffer buff = ByteBuffer.allocate(aid.length + 5);
			buff.put((byte) 0x00) // CLA Class
					.put((byte) 0xA4) // INS Instruction
					.put((byte) 0x04) // P1 Parameter 1
					.put((byte) 0x00) // P2 Parameter 2
		...
		public Response initializeUpdate(byte[] host_challenge) throws IOException {
			// 00A40400 0E 325041592E5359532E4444463031
			ByteBuffer buff = ByteBuffer.allocate(6+8); //其中8是host_challenge大小
		...
		public Response externalAuthentication(byte[] challenge,byte[] mac) throws IOException {
			// 00A40400 0E 325041592E5359532E4444463031
			ByteBuffer buff = ByteBuffer.allocate(21);
			buff.put((byte) 0x84) // CLA Class
		...
		public Response install(byte[] content,byte[] wrap) throws IOException {
			// 00A40400 0E 325041592E5359532E4444463031
			ByteBuffer buff = ByteBuffer.allocate( content.length + wrap.length + 5);
			buff.put((byte) 0x84) // CLA Class
					.put((byte) 0xE6) // INS Instruction
					.put((byte) 0x02) // P1 Parameter 1
					.put((byte) 0x00) // P2 Parameter 2
					.put((byte) (content.length + wrap.length))  //lc
					.put(content)
					.put(wrap);
					;
			return new Response(transceive(buff.array()));
		...
		public Response installForFinal(byte[] content,byte[] wrap) throws IOException {
			// 00A40400 0E 325041592E5359532E4444463031
			ByteBuffer buff = ByteBuffer.allocate( content.length + wrap.length + 5);
			buff.put((byte) 0x84) // CLA Class
					.put((byte) 0xE6) // INS Instruction
					.put((byte) 0x0C) // P1 Parameter 1
					.put((byte) 0x00) // P2 Parameter 2
					.put((byte) (content.length + wrap.length))  //lc
					.put(content)
					.put(wrap);
		...
		public Response loadByPart(byte p1,byte p2,byte [] content,byte[] wrap) throws IOException {
			// 00A40400 0E 325041592E5359532E4444463031
			ByteBuffer buff = ByteBuffer.allocate( content.length + wrap.length + 5);
			buff.put((byte) 0x84) // CLA Class
					.put((byte) 0xE8) // INS Instruction
					.put(p1) // P1 Parameter 1
					.put(p2) // P2 Parameter 2
					.put((byte)(content.length + wrap.length))  //lc
					.put(content)
					.put(wrap);
			System.out.println("<< "+HexUtils.bin2hex(buff.array())); //输出打印日志
			return new Response(transceive(buff.array()));
		}
         ...
         * @throws IOException
         */
		public Response send(byte cla,byte ins,byte p1,byte p2,byte [] content) throws IOException {
			// 00A40400 0E 325041592E5359532E4444463031
			ByteBuffer buff = ByteBuffer.allocate( content.length + 5);
			buff.put(cla) // CLA Class
					.put(ins) // INS Instruction
					.put(p1) // P1 Parameter 1
					.put(p2) // P2 Parameter 2
					.put((byte) content.length)  //lc
					.put(content);
			return new Response(transceive(buff.array()));
		}
		...
		public Response send(ByteBuffer buff) throws IOException {
			return new Response(transceive(buff.array()));
		}
		...
		public byte[] transceive(final byte[] cmd) throws IOException {
			try {
				System.out.println(">>" + HexUtils.bin2hex(cmd));
				//Log.d("发送命令:"," >> " + HexUtils.bin2hex(cmd));
				byte[] rsp = null;
				...
				System.out.println("<< " + HexUtils.bin2hex(rsp));
				//Log.d("返回信息:"," << " + HexUtils.bin2hex(rsp));
				return rsp;
